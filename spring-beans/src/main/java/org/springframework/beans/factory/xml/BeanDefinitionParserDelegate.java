...
public class BeanDefinitionParserDelegate {
     ...
     * if there were errors during parse. Errors are reported to the
     * {@link org.springframework.beans.factory.parsing.ProblemReporter}.
     * 在这里会看到对那些熟悉的BeanDefinition定义的处理，比如id、name、aliase等属性元素
     */
...
//这里只读取定义的<bean>中设置的class名字，然后载入到BeanDefinition中去，只是做个记录，并不涉及对象的实例化过程，对象的实例化实际上是在依赖注入时完成
...
//			property解析结果会被封装到propertyValue对象中，然后设置到BeanDefinitionHolder中去
...
//判断property的属性，是ref还是value，不允许同时是ref和value
...
//		如果还有子元素，进行子元素解析
    ...
    protected void parseCollectionElements(
        ...
        for (int i = 0; i < elementNodes.getLength(); i++) {
            ...
            if (node instanceof Element && !nodeNameEquals(node, DESCRIPTION_ELEMENT)) {
//                加入到target中，target是一个managedList，同时触发对下一层子元素的解析过程，这是一个递归的调用
